...
(define (make-carpeta drive string user ) ;el primer elemento es la ruta con la letra, segundo elemento lo ultimo agregado, tercer user, 4to documento 5to, tipo de doc, usuario
  (list drive string null null null (current-seconds))) ;este current seconds es la fecha de modificacion, aunque no se como mantenerlo fijo
...
(define cd
  (lambda(system)
    (lambda (path)
      (if(and(is-string path)(member path (map cadr (get-system-ruta system))))
         (make-system(get-system-name system);verdadero
                     (get-system-drive system)
                     (get-system-usuarios system)
                     (get-system-usuario-conectado system)
                     (get-system-drive-seleccionado system)
                     (cons(make-carpeta (string-append (string(car(get-system-drive-seleccionado system)))":/" path "/")path (get-system-usuario-conectado system))(get-system-ruta system)))

                     ;(if (and(>= (string-length (car(car(get-system-ruta system)))) 3) (equal? path "/"))
                      ;   (cons(make-carpeta(substring (car(car(get-system-ruta system) (-(string-length (car(car(get-system-ruta system))))5))) ;verdadero
                       ;  (string-length (car(car(get-system-ruta system)))))
                        ; path (get-system-usuario-conectado system))(get-system-ruta system))
         
;FALSO DEL SEGUNDO IF, DEJA TODO NORMAL, JUSTO COMO ESTÁ EN LA VERSION SIN SEGUNDO IF
         
                         ;(cons(make-carpeta (string-append (string(car(get-system-drive-seleccionado system)))":/" path "/") ;falso
                          ;                  path (get-system-usuario-conectado system))(get-system-ruta system))))
